#ifndef wasm_dynamic_INCLUDE_GUARD_MIR_ASTNODE
#define wasm_dynamic_INCLUDE_GUARD_MIR_ASTNODE
...
namespace mir
{
  ...
  template <ast_node T>
  T const *dyn_cast(ASTNode const *Node)
  {
    assert(is_a<T>(Node));
    return static_cast<T const *>(Node);
  }

  enum class InitializerExprKind
  {
    Constant,
    GlobalGet
  };
  ...
  template <initializer_expr T>
  bool is_a(InitializerExpr const *Node)
  {
    return T::classof(Node);
  }

  template <initializer_expr T>
  T *dyn_cast(InitializerExpr *Node)
  {
    assert(is_a<T>(Node));
    return static_cast<T *>(Node);
  }
  ...
  template <initializer_expr T>
  T const *dyn_cast(InitializerExpr const *Node)
  {
    assert(is_a<T>(Node));
    return static_cast<T const *>(Node);
  }
  ...
  class InitExprVisitorBase
  {
    ...
    template <typename T>
    using Ptr = std::conditional_t<Const, T const *, T *>;
    template <initializer_expr T>
    RetType castAndCall(Ptr<InitializerExpr> Expr)
    {
  ...
  public:
    RetType visit(Ptr<InitializerExpr> Expr)
    {
      ...
      {
      case InitExprKind::Constant:
        return castAndCall<Constant>(Expr);
      case InitializerExprKind::GlobalGet:
        return castAndCall<GlobalGet>(Expr);
      default:
        utility::unreachable();
      }
    }
