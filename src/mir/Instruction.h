#ifndef wasm_dynamic_INCLUDE_GUARD_MIR_IR
#define wasm_dynamic_INCLUDE_GUARD_MIR_IR
...
namespace mir
{
  ...
  bool is_a(Instruction const *Inst)
  {
  ...
  template <instruction T>
  T const *dyn_cast(Instruction const *Inst)
  {
    assert(is_a<T>(Inst));
...
{
  template <>
  struct ilist_callback_traits<mir::Instruction>
  {
...
namespace mir::instructions
{
  ...
  /////////////////////////////// IntUnaryOp /////////////////////////////////////
  enum class IntUnaryOperator : std::uint8_t
  {
    Eqz,
    Clz,
    Ctz,
...
namespace mir
{
  ...
  class InstVisitorBase
  {
    ...
    template <instruction T>
    RetType castAndCall(Ptr<Instruction> Inst)
    {
