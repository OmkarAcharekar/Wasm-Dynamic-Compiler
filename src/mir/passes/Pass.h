#ifndef wasm_dynamic_INCLUDE_GUARD_MIR_PASSES_PASS
#define wasm_dynamic_INCLUDE_GUARD_MIR_PASSES_PASS
...
template<typename T> concept module_pass = requires(T Pass) {
  ...
  template <module_pass T>
  struct SimpleModulePassDriver
  {
    T Pass;
  ...
  template <function_pass T>
  struct SimpleFunctionPassDriver
  {
    T Pass;
    using ArgType = std::conditional_t<
