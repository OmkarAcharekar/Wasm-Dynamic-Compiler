#ifndef wasm_dynamic_INCLUDE_GUARD_BYTECODE_TYPE
#define wasm_dynamic_INCLUDE_GUARD_BYTECODE_TYPE
...
namespace bytecode
{
  enum class ValueTypeKind : unsigned char
  {
    I32,
    I64,
    F32,
...
template <typename T> concept limit_like_type = requires(T Type) {
  ...
  enum class MutabilityKind : unsigned char
  {
    Const,
    Var
  };
...
template <>
struct fmt::formatter<bytecode::ValueType, char>
{
  ...
  auto format(bytecode::ValueType const &Type, Context &&CTX)
  {
    ...
    case Kind::I64:
      return format_to(CTX.out(), "i64");
    case Kind::F32:
      return format_to(CTX.out(), "f32");
    case Kind::F64:
      return format_to(CTX.out(), "f64");
    default:
      utility::unreachable();
    }
  }
...
template <>
struct fmt::formatter<bytecode::FunctionType, char>
{
  template <typename T>
  constexpr auto parse(T &&CTX) { return CTX.begin(); }
  template <typename Context>
...
namespace bytecode::detail
{
  struct LimitLikeTypeFormatterBase
  {
    ...
    {
      if (!Type.hasMax())
        return fmt::format_to(Out, "{{min {}}}", Type.getMin());
...
template <>
struct fmt::formatter<bytecode::MemoryType, char> : bytecode::detail::LimitLikeTypeFormatterBase
{
  template <typename T>
  constexpr auto parse(T &&CTX) { return CTX.begin(); }
  template <typename Context>
...
template <>
struct fmt::formatter<bytecode::TableType, char> : bytecode::detail::LimitLikeTypeFormatterBase
{
  template <typename T>
  constexpr auto parse(T &&CTX) { return CTX.begin(); }
  template <typename Context>
...
template <>
struct fmt::formatter<bytecode::GlobalType, char>
{
  template <typename T>
  constexpr auto parse(T &&CTX) { return CTX.begin(); }
  template <typename Context>
  auto format(bytecode::GlobalType const &Type, Context &&CTX) const
  {
    ...
    {
    case Kind::Const:
      return format_to(CTX.out(), "const {}", Type.getType());
    case Kind::Var:
      return format_to(CTX.out(), "var {}", Type.getType());
    default:
      utility::unreachable();
    }
  }
