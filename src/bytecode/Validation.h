#ifndef wasm_dynamic_INCLUDE_GUARD_BYTECODE_VALIDATION
#define wasm_dynamic_INCLUDE_GUARD_BYTECODE_VALIDATION
...
// clang-format off
#define wasm_dynamic_VALIDATION_MALFORMED_ERROR_KINDS                                 \
...
#define X(Name, Message) Name,
    wasm_dynamic_VALIDATION_MALFORMED_ERROR_KINDS
#undef X
  };
...
{
  template <>
  struct formatter<bytecode::validation::OperandStackElement>
  {
    using value_type = bytecode::validation::OperandStackElement;
    template <typename C>
    auto parse(C &&CTX) { return CTX.begin(); }
    template <typename C>
    auto format(value_type const &Type, C &&CTX)
    {
  ...
  template <>
  struct formatter<bytecode::validation::TypeError>
  {
    using value_type = bytecode::validation::TypeError;
    template <typename C>
    auto parse(C &&CTX) { return CTX.begin(); }
    template <typename C>
    auto format(value_type const &Error, C &&CTX)
    {
  ...
  template <>
  struct formatter<bytecode::validation::MalformedError>
  {
...
#undef wasm_dynamic_VALIDATION_MALFORMED_ERROR_KINDS
} // namespace fmt
