#ifndef wasm_dynamic_INCLUDE_GUARD_BYTECODE_INSTRUCTION
#define wasm_dynamic_INCLUDE_GUARD_BYTECODE_INSTRUCTION
...
concept instruction_in_category  = instruction<T>
  ...
  namespace detail
  {
    ...
    template <inst_with_no_immediate T>
    static T NoImmInstSingleton;
  } // namespace detail
...
#define X(Name, NameString, Category, OpcodeSeq, MemberArray)            \
  ...
  template <>                                                            \
  struct inst_trait<instructions::Name>                                  \
  {                                                                      \
